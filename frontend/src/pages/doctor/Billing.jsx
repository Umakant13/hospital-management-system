...
const DoctorBilling = () => {
    ...
    }, [page, rowsPerPage, searchQuery, statusFilter, dateFilter, user]);
    ...
    const fetchBills = async () => {
        try {
            ...
            // Note: In a real app, we should have a backend endpoint to get bills by doctor
            // For now, we'll fetch all and filter client-side if needed, or assume the backend handles it
            // Since the requirement is for the doctor to manage THEIR patient's billing, 
            // we might need to filter by the patients assigned to this doctor.
            const data = await billingService.getAllBills(params);

            // Filter bills to only show those for the doctor's patients
            // This requires patients to be loaded or checking against a list of patient IDs
            // For simplicity, we'll assume the doctor can see bills for patients they have access to
            // or we can filter if we have the patient list.
    ...
    const handleAddPayment = async (data) => {
        try {
            // Regular payment methods only (no Razorpay for doctors)
    ...
    const handleDownloadInvoicePDF = (bill) => {
        const doc = new jsPDF();
            ...
            <Dialog open={Boolean(viewBill)} onClose={handleCloseViewBill} maxWidth="sm" fullWidth>
                    ...
                    {viewBill && (
                        <Box>
                            ...
                            <List dense>
                                ...
                                <ListItem>
                                    <ListItemText
                                        primary={<Typography variant="h6">Balance Due</Typography>}
                                    />
                ...
                <DialogActions>
                    ...
                    <Button
                        variant="contained"
                        startIcon={<Download />}
                        onClick={() => handleDownloadInvoicePDF(viewBill)}
                    >
                        Download Invoice
                    </Button>
                </DialogActions>
